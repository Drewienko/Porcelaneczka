...
// ── useTweaks ──────────────────────────────────────────────────────────────
export function useTweaks<T>(defaults: T): [T, (key: keyof T, val: T[keyof T]) => void] {
  const [values, setValues] = useState<T>(defaults);
  const setTweak = useCallback((key: keyof T, val: T[keyof T]) => {
    setValues((prev) => ({ ...prev, [key]: val }));
    window.parent.postMessage({ type: '__edit_mode_set_keys', edits: { [String(key)]: val } }, '*');
  }, []);
...
interface RadioProps { label: string; value: string; options: (string | RadioOption)[]; onChange: (v: string) => void; }
...
interface SelectProps { label: string; value: string; options: (string | RadioOption)[]; onChange: (v: string) => void; }
...
interface ButtonProps { label: string; onClick: () => void; secondary?: boolean; }
export function TweakButton({ label, onClick, secondary = false }: ButtonProps) {
  return (
